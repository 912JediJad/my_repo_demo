def most_popular_foods(fav_foods):
    ...
    '''
    reversed_dict = {}
    for person in fav_foods:
        # print(person) # person
        # print(fav_foods[person]) # food
        if fav_foods[person] not in reversed_dict: # if food not in new dict
            reversed_dict[fav_foods[person]] = [person] # add food as key and list with person as value
        else:
            reversed_dict[fav_foods[person]].append(person) # add person to list of people who like that food

    print(reversed_dict)
    print(reversed_dict.keys())
    print(reversed_dict.values())

# def most_popular_foods2(fav_foods):
#     foods = {}
