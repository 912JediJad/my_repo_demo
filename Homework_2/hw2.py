...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    newlist2 = [x for x in alist if x < min or x > max]
    return newlist2
...
def topping1(dictionary):
    ...
    """
    if "ice cream" in dictionary.keys():
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    return {unique: strings.count(unique) for unique in strings}
...
def friend_list(friend_dictionary):
    ...
    """
    reversed_dict = {}
    for name in friend_dictionary: # name is key in original dict
        friend = friend_dictionary[name]
        if name not in reversed_dict:
            reversed_dict[name] = []
        if friend not in reversed_dict:
            reversed_dict[friend] = []
        reversed_dict[name].append(friend)
        reversed_dict[friend].append(name)
...
# print(friend_list({"Marty": "Cynthia", "Danielle": "Marty"}))
